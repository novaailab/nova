...
name = "nova-sdk-plugin-{plugin_name}"
version = "0.1.0"
description = "nova plugin for {plugin_name}"
authors = ["Your Name <your_email@example.com>"]
readme = "README.md"
keywords = ["nova", "sdk", "agents", "ai", "{plugin_name}"]
homepage = "https://ohmynova.dev/"
repository = "https://github.com/nova-sdk/nova"
packages = [
    {{ include = "nova_plugins/{plugin_name}" }},
]
...
def create_parameters_file(nova_plugins_dir: Path, plugin_name: str) -> None:
    """Create the parameters.py file with example parameters in the nova_plugins directory."""
...
'''
    ...
    with open(nova_plugins_dir / "parameters.py", "w") as f:
...
def create_service_file(nova_plugins_dir: Path, plugin_name: str, is_evm: bool, is_solana: bool = False) -> None:
    ...
    service_content = '''from nova.decorators.tool import Tool
from .parameters import ExampleQueryParameters, ExampleActionParameters
'''

    # Add chain-specific imports if needed
    if is_evm:
        service_content += '''from nova_wallets.evm import EVMWalletClient

'''
    elif is_solana:
        service_content += '''from nova_wallets.solana import SolanaWalletClient
...
'''
    ...
    with open(nova_plugins_dir / "service.py", "w") as f:
        f.write(service_content)

def create_init_file(nova_plugins_dir: Path, plugin_name: str, is_evm: bool, is_solana: bool = False) -> None:
...
'''.format(
    ...
    with open(nova_plugins_dir / "__init__.py", "w") as f:
        f.write(init_content)

def main():
    """Main function to create a new nova plugin.

    This script generates a new plugin package for the nova SDK with a standardized structure.
    ...
    """
    parser = argparse.ArgumentParser(description="Create a new nova plugin.")
    ...
    # Create nova_plugins directory and its contents
    nova_plugins_dir = plugin_dir / "nova_plugins" / plugin_name
    nova_plugins_dir.mkdir(parents=True, exist_ok=True)

    # Create all plugin files
    create_parameters_file(nova_plugins_dir, plugin_name)
    create_service_file(nova_plugins_dir, plugin_name, is_evm, is_solana)
    create_init_file(nova_plugins_dir, plugin_name, is_evm, is_solana)
    ...
    readme_content = f"""# {plugin_name} Plugin for nova SDK

A plugin for the nova SDK that provides {plugin_name} functionality.

## Installation

```bash
# Install the plugin
poetry add nova-sdk-plugin-{plugin_name}
...
```python
from nova_plugins.{plugin_name} import {python_name}, {class_base}PluginOptions
